---
title: "Poon Hill Trek"
region: "Annapurna"
regionSlug: "annapurna"
country: "Nepal"
countrySlug: "nepal"
difficulty: "Easy"
durationDays: 4
maxAltitudeM: 3210
distanceKm: 33
bestSeason: ["March", "April", "May", "September", "October", "November", "December"]
startingPoint: "Nayapul (near Pokhara)"
endingPoint: "Nayapul"
groupSize: "2–20"
shortDescription: "Nepal's classic short trek — a sunrise viewpoint over the Annapurna and Dhaulagiri ranges, reachable in a long weekend."
highlights:
  - "Sunrise from Poon Hill (3,210m), one of Nepal's most famous viewpoints"
  - "Ghorepani and Tadapani, two of the Annapurna region's most scenic teahouse villages"
  - "Rhododendron forest in spectacular bloom each spring"
  - "Comfortably achievable for first-time trekkers"
itinerary:
  - day: 1
    title: "Pokhara to Nayapul, trek to Tikhedhunga"
    description: "Drive from Pokhara to Nayapul and trek to Tikhedhunga."
  - day: 2
    title: "Tikhedhunga to Ghorepani"
    description: "A steady climb through rhododendron forest to Ghorepani (2,860m)."
  - day: 3
    title: "Poon Hill sunrise, trek to Tadapani"
    description: "A pre-dawn climb to Poon Hill for sunrise over the Annapurna and Dhaulagiri ranges, then trek on to Tadapani."
  - day: 4
    title: "Tadapani to Nayapul, drive to Pokhara"
    description: "Descend to Nayapul and drive back to Pokhara, concluding the trek."
howToReach: "Nayapul is about an hour's drive from Pokhara, itself a short flight or 6–7 hour drive from Kathmandu."
permits: "A TIMS card and Annapurna Conservation Area Project (ACAP) permit are required, easily arranged in Pokhara."
tags: ["short trek", "easy", "sunrise viewpoint", "beginner-friendly"]
heroImage:
  src: "/images/treks/poon-hill.jpg"
  alt: "Sunrise over the Annapurna range from Poon Hill"
  author: "Usha Pande"
  license: "CC BY-SA 4.0"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:Sunrise_behind_the_Annapurna_range._View_from_Poonhill.jpg"
seoTitle: "Poon Hill Trek | 4-Day Itinerary via Ghorepani | Tripcamps"
seoDescription: "Plan the Poon Hill trek: an easy 4-day itinerary via Ghorepani for sunrise views of the Annapurna and Dhaulagiri ranges."
featured: false
---

Poon Hill is Nepal's answer to "what's the best mountain view for the least effort" — a short, teahouse-supported trek that culminates in a pre-dawn climb to a ridge-top viewpoint with an unobstructed panorama of the Annapurna and Dhaulagiri ranges, achievable comfortably over a long weekend.

The trek's other charm is the walk itself: the trail between Tikhedhunga and Ghorepani climbs through some of the Annapurna region's most striking rhododendron forest, spectacular in bloom each spring, past stone-built villages that have supported trekkers for decades.

Because it's short, requires no camping gear, and tops out at just over 3,200m, Poon Hill is regularly recommended as Nepal's best introductory trek — a genuine taste of Himalayan scenery and teahouse trekking life without a major time or fitness commitment.
