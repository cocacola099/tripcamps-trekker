---
title: "Upper Mustang Trek"
region: "Mustang"
regionSlug: "mustang"
country: "Nepal"
countrySlug: "nepal"
difficulty: "Moderate"
durationDays: 10
maxAltitudeM: 4010
distanceKm: 90
bestSeason: ["March", "April", "May", "June", "July", "August", "September", "October", "November"]
startingPoint: "Jomsom"
endingPoint: "Jomsom"
groupSize: "2–12"
shortDescription: "A trek into a former forbidden kingdom, through eroded high-desert canyons to the walled city of Lo Manthang."
highlights:
  - "Lo Manthang, Mustang's walled former royal capital"
  - "Centuries-old cave complexes carved into cliff faces"
  - "A rain-shadow climate that keeps the region trekkable even in monsoon"
  - "Tibetan Buddhist culture preserved in a form increasingly rare even in Tibet"
itinerary:
  - day: 1
    title: "Fly Kathmandu/Pokhara to Jomsom, trek to Kagbeni"
    description: "Fly into Jomsom and trek a short distance to Kagbeni, the gateway to Upper Mustang."
  - day: 2
    title: "Kagbeni to Chele"
    description: "Enter the restricted area, trekking through the first stretches of high-desert terrain."
  - day: 3
    title: "Chele to Syangboche"
    description: "Continue through eroded canyon terrain to Syangboche."
  - day: 4
    title: "Syangboche to Ghami"
    description: "Trek past chortens and mani walls to the village of Ghami."
  - day: 5
    title: "Ghami to Charang"
    description: "Continue to Charang, home to an impressive dzong (fort) and monastery."
  - day: 6
    title: "Charang to Lo Manthang"
    description: "Reach Lo Manthang (3,840m), Mustang's walled former royal capital."
  - day: 7
    title: "Explore Lo Manthang"
    description: "A full day exploring the walled city's palace, monasteries, and surrounding cave complexes."
  - day: 8
    title: "Lo Manthang to Drakmar"
    description: "Begin the return journey via an alternate route through Drakmar's red cliffs."
  - day: 9
    title: "Drakmar to Chhusang"
    description: "Continue the descent back toward Kagbeni."
  - day: 10
    title: "Chhusang to Jomsom, fly to Pokhara/Kathmandu"
    description: "Complete the trek to Jomsom and fly out, concluding the trip."
howToReach: "Jomsom is reached by a short scenic flight from Pokhara, itself a short flight or drive from Kathmandu."
permits: "A restricted area permit for Upper Mustang (with a required minimum group size) and Annapurna Conservation Area Project (ACAP) permit are both required, arranged through a registered trekking agency."
tags: ["restricted area", "moderate", "high desert", "Tibetan culture"]
heroImage:
  src: "/images/treks/upper-mustang.jpg"
  alt: "The high-desert landscape of Upper Mustang near Lo Manthang"
  author: "Sunuwargr"
  license: "CC BY-SA 4.0"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:A_Lens_into_the_Forbidden_Kingdom.jpg"
seoTitle: "Upper Mustang Trek | 10-Day Itinerary & Guide | Tripcamps"
seoDescription: "Plan the Upper Mustang trek to Lo Manthang: 10-day itinerary, permits, and why Mustang is trekkable even during monsoon."
featured: true
---

Upper Mustang was an independent kingdom until 2008 and remained closed to outside visitors until 1992, and it still feels like one of the more genuinely remote places accessible by trek in the Himalaya. Sitting in the rain shadow of the Annapurna and Dhaulagiri ranges, the landscape is a high-altitude desert of eroded cliffs, wind-carved canyons, and ochre-red rock formations utterly unlike the lush valleys most trekkers associate with Nepal.

The trek's destination, Lo Manthang, is Mustang's walled former royal capital — a compact, still-inhabited medieval city with a royal palace, several significant monasteries, and centuries-old cave complexes carved into nearby cliffs, some still containing preserved manuscripts and artwork. The Tibetan Buddhist culture here has developed with less outside disruption than in Tibet itself, making it a genuinely rare cultural destination.

Mustang's rain-shadow location gives it one of the widest trekking windows in Nepal — even July and August, monsoon season everywhere else, are viable here, making it one of the few worthwhile Nepal treks to consider outside the standard spring and autumn windows.
