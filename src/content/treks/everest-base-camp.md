---
title: "Everest Base Camp Trek"
region: "Khumbu"
regionSlug: "khumbu"
country: "Nepal"
countrySlug: "nepal"
difficulty: "Difficult"
durationDays: 12
maxAltitudeM: 5364
distanceKm: 130
bestSeason: ["March", "April", "May", "September", "October", "November"]
startingPoint: "Lukla"
endingPoint: "Lukla"
groupSize: "2–12"
shortDescription: "The world's most iconic trek — a two-week journey through Sherpa country to the foot of the world's highest mountain."
highlights:
  - "Standing at Everest Base Camp itself, beneath the Khumbu Icefall"
  - "Kala Patthar (5,644m), the trek's best viewpoint of Everest's summit"
  - "Namche Bazaar, the bustling Sherpa trading capital of the Khumbu"
  - "Tengboche Monastery, the region's most important Buddhist monastery"
itinerary:
  - day: 1
    title: "Fly to Lukla, trek to Phakding"
    description: "A short, dramatic flight from Kathmandu to Lukla's mountain airstrip, then an easy 3-hour walk down to Phakding."
  - day: 2
    title: "Phakding to Namche Bazaar"
    description: "Cross the Dudh Kosi river on suspension bridges and climb steadily into Namche Bazaar (3,440m), the Khumbu's trading hub."
  - day: 3
    title: "Acclimatisation day in Namche"
    description: "A rest day with an acclimatisation hike to the Everest View Hotel or Khumjung village."
  - day: 4
    title: "Namche to Tengboche"
    description: "Trek to Tengboche (3,860m), home to the Khumbu's most important monastery, with first close views of Everest and Ama Dablam."
  - day: 5
    title: "Tengboche to Dingboche"
    description: "Continue up the valley to Dingboche (4,410m), entering high alpine terrain."
  - day: 6
    title: "Acclimatisation day in Dingboche"
    description: "A rest day with a hike to Nangkartshang viewpoint for wide Himalayan views."
  - day: 7
    title: "Dingboche to Lobuche"
    description: "Trek past the Khumbu Glacier's memorial cairns to Lobuche (4,940m)."
  - day: 8
    title: "Lobuche to Gorak Shep, visit Everest Base Camp"
    description: "Reach Gorak Shep (5,164m) and continue to Everest Base Camp (5,364m) itself before returning to Gorak Shep."
  - day: 9
    title: "Kala Patthar sunrise, descend to Pheriche"
    description: "A pre-dawn climb to Kala Patthar (5,644m) for the trek's best Everest summit view, then descend to Pheriche."
  - day: 10
    title: "Pheriche to Namche Bazaar"
    description: "A long descending day back to Namche Bazaar."
  - day: 11
    title: "Namche to Lukla"
    description: "Complete the descent back to Lukla."
  - day: 12
    title: "Fly to Kathmandu"
    description: "Morning flight from Lukla back to Kathmandu, concluding the trek."
howToReach: "Lukla is reached by a 30–40 minute flight from Kathmandu (weather-dependent, with possible delays), the standard start and end point for the trek. Kathmandu is served by direct international flights from major Indian and Southeast Asian cities."
permits: "A Sagarmatha National Park entry permit and a Khumbu Pasang Lhamu rural municipality entry fee are both required and are usually arranged by your trek operator or in Kathmandu. A TIMS card is not required within the Khumbu but is worth confirming with your operator."
tags: ["classic Himalaya", "difficult", "Sherpa culture", "flagship trek"]
heroImage:
  src: "/images/treks/everest-base-camp.jpg"
  alt: "Everest Base Camp trail with the Khumbu Icefall in view"
  author: "Icicles Adventure Treks & Tours (P.) Ltd."
  license: "CC BY-SA 4.0"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:A_group_of_trekkers_on_the_way_to_Everest_Base_Camp.jpg"
seoTitle: "Everest Base Camp Trek | 12-Day Itinerary & Complete Guide | Tripcamps"
seoDescription: "Plan the Everest Base Camp trek: full 12-day itinerary via Namche and Kala Patthar, best season, permits, and how to reach Lukla."
featured: true
---

Everest Base Camp needs little introduction: it's the trek that put Himalayan trekking on the global map, and remains the benchmark by which every other high-altitude trek is measured. The route from Lukla climbs steadily through Sherpa villages and pine forest, gaining altitude gradually enough to allow proper acclimatisation, before the terrain opens into the stark, high-alpine world around the Khumbu Glacier.

Namche Bazaar and Tengboche Monastery are highlights in their own right — a bustling trading town built into a natural amphitheatre, and Nepal's most significant Buddhist monastery, respectively — before the trail's final push to the base camp itself, sitting at the foot of the Khumbu Icefall, the notoriously unstable first obstacle on Everest's climbing route. Most trekkers agree the actual best view of Everest comes not from base camp (where the mountain itself is partially hidden by nearer peaks) but from Kala Patthar, a short, steep pre-dawn climb above Gorak Shep.

Twelve days is enough for proper acclimatisation with two dedicated rest days, but altitude sickness remains a real risk above 4,000m — a measured pace, rather than rushing between famous viewpoints, is the single biggest factor in a safe and successful trip.
