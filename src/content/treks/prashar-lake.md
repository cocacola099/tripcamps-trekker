---
title: "Prashar Lake Trek"
region: "Himachal Pradesh"
regionSlug: "himachal-pradesh"
difficulty: "Easy"
durationDays: 2
maxAltitudeM: 2730
distanceKm: 15
bestSeason: ["March", "April", "May", "June", "September", "October", "November", "December"]
startingPoint: "Baggi village (Mandi)"
endingPoint: "Baggi village"
groupSize: "2–20"
shortDescription: "A short, family-friendly trek to a floating lake and a distinctive three-tiered pagoda temple dedicated to sage Prashar."
highlights:
  - "A floating island said to shift position on the lake's surface"
  - "A 13th-century, three-tiered pagoda-style temple dedicated to sage Prashar"
  - "Panoramic views of the Dhauladhar and Pir Panjal ranges from the ridge above the lake"
  - "One of the easiest overnight treks in Himachal, suitable for families"
itinerary:
  - day: 1
    title: "Baggi village to Prashar Lake"
    description: "A climb of roughly 7–8km through forest and open ridge to the lake and temple at 2,730m."
  - day: 2
    title: "Prashar Lake to Baggi village"
    description: "Explore the lake and temple at sunrise before descending back to Baggi."
howToReach: "Baggi village is about 25km from Mandi town, itself roughly 8 hours by road from Delhi via Chandigarh. Mandi is the standard base for organising this trek."
permits: "No special permit is required. This is a beginner and family-friendly trek with a well-marked trail and a temple guesthouse option at the top for those not wanting to camp."
tags: ["easy", "family-friendly", "temple trek", "short trek"]
heroImage:
  src: "/images/treks/prashar-lake.jpg"
  alt: "Floating Prashar Lake and pagoda temple"
  author: "Biswajit Majumdar"
  license: "CC BY-SA 3.0"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:Prashar_lake_and_Temple,Himachal_Pradesh_-_panoramio.jpg"
seoTitle: "Prashar Lake Trek — 2-Day Itinerary from Mandi | Tripcamps"
seoDescription: "Plan the Prashar Lake trek near Mandi: an easy 2-day itinerary to a floating lake and pagoda temple, ideal for families."
featured: false
---

Prashar Lake is as much a pilgrimage and curiosity destination as it is a trek — a small, near-circular lake at 2,730m with a patch of vegetation that appears to drift across its surface, and a striking three-tiered pagoda-style temple on its bank, built in the 13th century and dedicated to the sage Prashar.

The trail up from Baggi village is short and gentle by Himalayan standards, gaining altitude gradually through forest before opening onto ridgeline views of both the Dhauladhar range to the north and the Pir Panjal to the south — a wide panorama for very little trekking effort.

Because the route is well marked and there's a guesthouse option at the temple itself, Prashar Lake is one of the more approachable overnight treks in Himachal Pradesh, regularly recommended for families or trekkers easing into overnight mountain trips.
