---
title: "Mardi Himal Trek"
region: "Annapurna"
regionSlug: "annapurna"
country: "Nepal"
countrySlug: "nepal"
difficulty: "Moderate"
durationDays: 6
maxAltitudeM: 4500
distanceKm: 40
bestSeason: ["March", "April", "May", "September", "October", "November", "December"]
startingPoint: "Kande (near Pokhara)"
endingPoint: "Pokhara"
groupSize: "2–15"
shortDescription: "A ridge-top trek to a high viewpoint directly beneath Machhapuchhre (Fishtail), one of the Annapurna region's newer and quieter routes."
highlights:
  - "Close-up views of Machhapuchhre's fishtail summit, rarely seen this near on other treks"
  - "A narrow, dramatic ridge-top trail unlike most Annapurna approaches"
  - "A relatively new trekking route, opened to the public only in 2012"
  - "Noticeably quieter than Poon Hill or Annapurna Base Camp"
itinerary:
  - day: 1
    title: "Pokhara to Kande, trek to Deurali"
    description: "A short drive to the trailhead at Kande, then a climb to the ridge campsite at Deurali."
  - day: 2
    title: "Deurali to Forest Camp"
    description: "Continue along the forested ridge to Forest Camp (2,600m)."
  - day: 3
    title: "Forest Camp to High Camp"
    description: "Climb above the treeline to High Camp (3,580m), with Machhapuchhre now dominating the skyline."
  - day: 4
    title: "High Camp to Mardi Himal viewpoint and back"
    description: "A pre-dawn climb to the upper viewpoint (around 4,500m) for close views of Machhapuchhre and the Annapurna range, then descend to Low Camp."
  - day: 5
    title: "Low Camp to Siding village"
    description: "Descend off the ridge into the village of Siding."
  - day: 6
    title: "Siding to Pokhara"
    description: "Drive from Siding back to Pokhara, concluding the trek."
howToReach: "Kande is about an hour's drive from Pokhara, itself a short flight or 6–7 hour drive from Kathmandu."
permits: "A TIMS card and Annapurna Conservation Area Project (ACAP) permit are required, arranged in Pokhara."
tags: ["ridge trek", "moderate", "quieter alternative", "Machhapuchhre"]
heroImage:
  src: "/images/treks/mardi-himal.jpg"
  alt: "Tadapani, a forested village on the southern Annapurna approach near Mardi Himal"
  author: "Wassuprai"
  license: "Public domain"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:06_Tadapani.jpg"
seoTitle: "Mardi Himal Trek — 6-Day Itinerary & Guide | Tripcamps"
seoDescription: "Plan the Mardi Himal trek near Pokhara: a 6-day itinerary to a ridge-top viewpoint beneath Machhapuchhre, quieter than Poon Hill."
featured: false
---

Mardi Himal is one of the newer additions to Nepal's trekking map, only opened to trekkers in 2012, and it remains noticeably quieter than the Annapurna region's more established routes despite matching them for scenery. The trail follows a narrow forested ridge rather than a valley, climbing steadily to a high viewpoint that sits almost directly beneath Machhapuchhre — the sacred, fishtail-shaped peak that dominates views from Pokhara itself.

Because the route stays on the ridge rather than dropping into a village-lined valley, the trekking experience here feels notably more remote than Poon Hill or even Annapurna Base Camp, despite a similarly short overall duration. The final viewpoint above High Camp offers a rare close-up angle on Machhapuchhre's summit alongside wider Annapurna range views.

At six days and moderate difficulty, Mardi Himal is a good choice for trekkers who've already done Poon Hill and want a similarly short trip with a quieter trail and a different mountain to admire.
