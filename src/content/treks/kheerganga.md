---
title: "Kheerganga Trek"
region: "Himachal Pradesh"
regionSlug: "himachal-pradesh"
country: "India"
countrySlug: "india"
difficulty: "Easy"
durationDays: 2
maxAltitudeM: 2960
distanceKm: 12
bestSeason: ["March", "April", "May", "June", "September", "October", "November", "December"]
startingPoint: "Barshaini (Parvati Valley)"
endingPoint: "Barshaini"
groupSize: "2–20"
shortDescription: "A popular Parvati Valley trek to a meadow with natural hot springs, framed by pine forest and mountain views."
highlights:
  - "Natural hot spring pools at the top, believed sacred to Lord Shiva"
  - "One of the most sociable treks in Himachal, with a well-established campsite and food stalls"
  - "Forest trail alongside the Parvati river for much of the route"
  - "Short enough to combine with a wider Parvati Valley trip via Kasol and Tosh"
itinerary:
  - day: 1
    title: "Barshaini to Kheerganga"
    description: "A climb of roughly 6km through pine and cedar forest alongside the Parvati river to the meadow campsite at Kheerganga (2,960m)."
  - day: 2
    title: "Kheerganga to Barshaini"
    description: "Enjoy the hot springs at sunrise before descending back to Barshaini."
howToReach: "Barshaini is about 4 hours from Bhuntar (the nearest airport, near Kullu) and roughly 12 hours by road from Delhi via Manali or Mandi. Kasol, a short drive from Barshaini, is the usual base for organising the trek."
permits: "No special permit is required. This is a well-trodden, easy trail suitable for beginners, though weekends can be crowded."
tags: ["hot springs", "easy", "Parvati Valley", "weekend trek"]
heroImage:
  src: "/images/treks/kheerganga.jpg"
  alt: "Kheerganga meadow above the Parvati Valley"
  author: "Nikhil.m.sharma"
  license: "CC BY-SA 4.0"
  sourceUrl: "https://commons.wikimedia.org/wiki/File:Kheerganga,_Himachal_Pradesh.jpg"
seoTitle: "Kheerganga Trek — 2-Day Itinerary via Barshaini | Tripcamps"
seoDescription: "Plan the Kheerganga trek in the Parvati Valley: an easy 2-day itinerary to a meadow with natural hot springs."
featured: false
---

Kheerganga has become the Parvati Valley's signature trek, and the reason is simple: a moderate half-day climb through forest ends at a meadow campsite with genuine natural hot spring pools, said to be where Lord Shiva meditated for thousands of years according to local legend. Soaking in the springs at dusk or sunrise, with the surrounding peaks catching the light, is the trek's main draw.

The trail itself is straightforward and well used, following the Parvati river through pine and cedar forest from the roadhead at Barshaini — making this one of the more sociable treks in the region, with a small but established string of food stalls and guesthouses at the top rather than pure wilderness camping.

Because it's short and easy, Kheerganga pairs naturally with a longer trip through the wider Parvati Valley, based out of Kasol or the nearby village of Tosh, both popular backpacker hubs with easy onward transport.
